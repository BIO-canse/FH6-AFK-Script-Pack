...
`v1.2.7` 更新了刷分调教、收藏蓝图和难度设置说明，并新增等待时间调整菜单。默认等待仍然偏保守；性能强、加载快的电脑可以在主程序菜单 `4. 调整等待时间` 里自行缩短。
...
车库里需要有一辆用于刷蓝图的改装 `IMPREZA 22B-STI VERSION`，性能分必须是 `900`。推荐使用调教代码 `446 536 222`；也可以自己找能稳定跑到指定时间范围的调教，但不要盲目追求最快，太快会因为倍率到不了 `9` 导致少拿技术点，太慢也会错过脚本时序。程序会选择车辆列表排序最前的 `900` 分普通目标车，也就是状态 `2` 的指定车型。
...
蓝图代码 `170 516 901` 需要已经加入“我的收藏”，并且处于“我的收藏”的第一位。程序进入收藏蓝图列表后会直接选择第一项。

![我的收藏第一位蓝图示例](assets/favorite-blueprint-first.png)

刷一次蓝图后，游戏结算会显示实际局内用时。这个实际局内净时间不要小于 `13` 秒；太快会导致倍率不到 `9`，太慢会错过脚本时序。脚本默认会从按下 `Enter` 开始计时，等待 `28` 秒后按 `X`；这个等待可以在主程序菜单 `4. 调整等待时间` 里修改。
