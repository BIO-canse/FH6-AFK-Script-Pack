...
`v1.2.9` 更新了默认推荐蓝图参数，并把刷图收益、蓝图净时间、刷分车性能分前置为可配置项。默认等待仍然偏保守；性能强、加载快的电脑可以在主程序菜单 `4. 调整等待时间` 里自行缩短。
...
6. 确认后程序默认会等待 10 秒，这段时间切回游戏窗口。等待结束时需要处于“大世界”可操作状态，参考下图；这个启动等待可以在菜单 `4. 调整等待时间` 里修改。
...
主程序开始的运行模式菜单里还有 `4. 调整等待时间`。默认等待偏保守；性能强、加载快的电脑可以适当缩短。可调项包括：开始确认后的等待、全自动阶段间隔、滚动后等待、UI OCR 前等待、复用 UI 坐标缓存后的点击补偿、以及刷技术点时 `Enter` 后等待多久按 `X`。如果出现吞键、页面没加载完或 OCR 太早，调回默认或加长等待。
...
车库里需要有一辆用于刷蓝图的改装 `IMPREZA 22B-STI VERSION`。刷分车性能分默认按 `834` 识别，可以在运行模式 `3` 重设设置里修改。程序会选择车辆列表排序最前、性能分等于当前配置值、且没有 `全新` 标记的普通目标车，也就是状态 `2` 的指定车型。
...
![难度设置示例](assets/difficulty-settings.png)

- 智能车手难度建议设为“熟练者”，避免赛事开始前弹出额外询问，影响自动流程。
- 驾驶辅助预设使用“自定义”。
- 刹车设为“开启防抱死系统”。
- 转向设为“自动转向”。
- 牵引力控制系统、稳定控制系统、倒转、起跑控制保持“开启”。
- 换挡设为“手动”。
- 行驶路线设为“实心”。
- 损坏及轮胎损耗设为“无”。
...
- 当前默认推荐蓝图需要开启自动转向、手动挡、牵引力控制系统和稳定控制系统。如果自己换了其它蓝图，请按蓝图分享者提供的设置调整。
- 在游戏设置里进入“抬头显示”，把“技术动画”关闭，避免刷图结算和连击动画干扰固定等待。

### 3. 收藏蓝图

蓝图容易失效，请自己找到当前可用、收益稳定的刷技术点蓝图，加入“我的收藏”，并且放在“我的收藏”的第一位。可以到这个视频下面找当前可用蓝图：[Bilibili 蓝图参考](https://www.bilibili.com/video/BV1TwjN6NELh/?vd_source=6832a0a0a4121c57db1c741922b502bd)。目前各种蓝图封得很快，推荐多看评论区玩家复制的蓝图码。

当前默认推荐蓝图码是 `123 675 780`，默认按每轮 `50` 技术点、蓝图净时间 `101` 秒、循环额外时间 `25.3` 秒、完整循环估算 `126.3` 秒、刷分车性能分 `834` 预填。如果你换了其它蓝图或车辆，需要在运行模式 `3` 里改掉这些值。

![我的收藏第一位蓝图示例](assets/favorite-blueprint-first.png)

正式挂机前，先手动跑一次你选好的蓝图，记录每次跑图获得的技术点、结算显示的蓝图净时间和分享者要求的游戏设置。运行模式 `3` 重设设置时，把技术点收益、蓝图净时间、刷分车性能分填进去。程序会用“净时间 + 当前循环额外时间”自动计算完整循环时间和刷满预计时间；默认 `Enter -> X` 等待为 `115` 秒，也可以在菜单 `4` 中调整。

### 4. 首次运行设置

第一次运行或重设设置时，程序会让你做这些准备：

1. 先填写刷图关键参数：每次跑图获得的技术点、你自己跑一次测得的蓝图净时间、刷分车性能分、点技能/删车识别用车辆性能分。
2. 进入下面这种“我的车辆”选车页面。
3. 填写当前屏幕能完整看到几列车、几行车。只算完整显示的车辆格子，不完整的不要算；左侧车辆详情栏、最左/最右边缘露出的半个格子都不要算。
4. 按提示用鼠标画框，精确框住“我的车辆”页面里所有完整可见车辆格子的整体区域。
   - 只框右侧车辆格子区域。
   - 不要框左侧车辆详情栏。
   - 不要框顶部制造商标签栏或底部按键提示。
   - 不要把边缘不完整的半格框进去。
5. 输入当前已有技术点数量。
6. 输入当前 CR 点数。这个值只在本次运行中使用，不写入永久配置。
7. 最后按 `Enter` 确认开始。确认前检查：
...
确认开始后，程序默认会等待 10 秒才真正执行；如果你在菜单 `4. 调整等待时间` 修改过启动等待，则按修改后的值执行。
...
4. 自动删除已点完且可删的多余车辆。
5. 找到车辆列表排序最前、性能分等于当前配置的指定车型，用它进入刷技术点蓝图。
6. 按当前车库情况自动补买目标车。
7. 自动运行刷技术点蓝图；普通模式把技术点补到 `999`，快速验证模式只补到达/超过 `100`。默认每轮按 `+50` 技术点计算。
8. 返回车库，继续下一轮。
