# 全自动开蓝图车辆选择重构文档

## 范围

本轮只重构 `FH6FullAuto.exe` 中“选择开蓝图车辆”的车辆列表查找流程。

暂不重构：

- 大世界进入车库标准位。
- 买车前置流程。
- 创意中心和蓝图选择流程。
- 自动点技术点、自动删车子程序。

## 当前规则

用户修正规则：“5的规则改一下，现在不要直接写5（避免覆盖掉），而是选择性能分（一定是三位数）最高的指定类型车辆”。

进一步确认：“900现在就是正常的2车”。

最新修正规则：“不要选择最高性能分得了，只选择列表排序最前的性能分为900的2。”

当前落实规则：

- 刷分车性能分已经配置化，默认 `834`；它仍然只是 `PerformanceScore`，不能把格子写成状态 `5`。
- 指定车型本身仍然是状态 `2`，如果带有效 `全新` 是状态 `3`，如果是可删车是状态 `4`。
- 不再因为性能分符合刷分车配置就把格子写成状态 `5`。
- 开蓝图时只选择状态 `2` 且 `PerformanceScore == 当前配置刷分车性能分` 的指定车型。
- 如果有多个符合配置刷分性能分的状态 `2`，按列表顺序选择最前的一个，也就是先列后行的最小位置。
- 状态 `3`、状态 `4`、性能分不符合当前配置的目标车都不能作为开蓝图车辆。
- 如果虚拟表里没有符合当前配置刷分性能分的状态 `2` 指定车型，程序报错停止，避免默认上错车。

## 稳定对象

- `VirtualVehicleList`：读取删车后的虚拟表，保存 `IsTarget`、`NewState`、`PerformanceScore` 和当前 offset。
- `BuildVehicleGridObservation()` / `ApplyVehicleGridObservation()`：定表 OCR 时统一识别 `IMPREZA 22B-STI`、`全新`、`斯巴鲁` 和三位数性能分，并写入虚拟表。
- `DrivePlanner`：开蓝图车辆选择阶段唯一的下一步决策者。
- 执行器：沿用键盘移动逻辑，根据目标全局列和当前 offset 生成 `Left` / `Right` / `Down` 路径。

## 动作集合

```text
Select(globalCell)
Scroll(ticks, reason)
Observe(reason)
UseDefault(reason) 旧兼容路径，表驱动模式不应走到这里
```

含义：

- `Select`：虚拟表已经有列表最前、符合当前配置刷分性能分的状态 `2` 候选，直接生成键盘路径。
- `Scroll`：兼容非表驱动路径时使用。
- `Observe`：兼容非表驱动路径时 OCR 当前车辆列表区域并写表。
- `UseDefault`：旧兼容路径；全自动定表模式中没有候选时应报错，不默认第一格。

## 决策顺序

```text
if 虚拟表内存在 StateCode == 2 且 PerformanceScore == 当前配置刷分车性能分 的指定车型:
    Select(列表顺序最前的格子)

else if 当前是全自动定表模式:
    Observe/Fail(定表内没有符合当前配置刷分性能分的状态 2 开蓝图候选，停止排查)

else:
    走旧兼容 OCR/滚动路径
```

## 检查点

- 刷分车在虚拟表里必须仍显示为状态 `2`，不能显示或保存为状态 `5`。
- 状态 `3` / `4` 不能被性能分覆盖。
- 开蓝图目标通过 `StateCode == 2 && PerformanceScore == 当前配置刷分车性能分` 选择，而不是通过 `NewState=drive` 或最高性能分选择。
- 叠加层小圆只是“当前准备选择的开蓝图目标”高亮，不代表状态码 `5`。
